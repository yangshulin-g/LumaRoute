...
**Goal:** 在不新增产品能力、不合入 v0.2 播放器扩展的前提下，为现有 LumaRoute v0.1 建立可复现的播放启动换线、真实 Jellyfin、真实 mpv、三平台安装包和 macOS/Windows 实机闭环证据，交付仅供内部技术验证的未签名 Alpha。
...
**Tech Stack:** pnpm 10.15.0、Node.js 22、TypeScript 5.9、Vitest 4、Testcontainers 11、Rust stable、Tokio、Tauri 2、独立 mpv 0.41 系列 JSON IPC、Playwright、GitHub Actions、Windows x64、macOS Intel/Apple Silicon、Linux x64。
...
- 只有全量质量门、专用播放换线、强制 Jellyfin 容器契约、三平台产物与 SHA-256、macOS/Windows Emby/Jellyfin 实机闭环、真实 mpv 样片和零凭证泄漏全部满足后，才能标记 Internal Alpha 通过。
...
- 本机 Docker/OrbStack socket 不存在；`pnpm test:integration` 为 2 个测试文件通过、3 个测试中 1 个跳过，跳过项正是 live Jellyfin contract。
...
- Windows x64、macOS Intel 与 Linux x64 的 `mpv.lock.json` 状态为 `archive-sealed`；只有 Apple Silicon 为 `qualified`。Internal Alpha 前必须以原生 CI 结果补齐真实资格证据，不能直接把状态文本改成通过。
...
```markdown
- [x] Temporary Jellyfin contract executes without skips.
  - Evidence: `tests/integration/jellyfin-contract.test.ts`; https://github.com/owner/repository/actions/runs/123456789
  - Limitations: controlled fixture and ephemeral credentials only
```
...
## Task 2: 让 Jellyfin 容器契约不可跳过

**可独立验收：** Linux CI 使用不可变 Jellyfin digest 实际启动容器，并完成认证、媒体库浏览、搜索、播放计划和 Started/Stopped 进度上报；要求容器的环境若运行时不可用则明确失败，本机普通开发仍可记录环境限制。
...
- Produces: exact `JELLYFIN_IMAGE`; fail-closed `requireContainerRuntime(available)`; live contract including `getPlaybackPlan`.

- [ ] **Step 1: 写 digest 和强制运行时的失败测试**
...
- [ ] **Step 4: 在 Linux quality job 建立强制容器门**
...
- name: Integration contracts without live container
  if: runner.os != 'Linux'
  run: pnpm test:integration

- name: Required live Jellyfin contract
  if: runner.os == 'Linux'
  run: pnpm test:integration
  env:
    LUMAROUTE_REQUIRE_CONTAINER: '1'
    LUMAROUTE_JELLYFIN_IMAGE: jellyfin/jellyfin@sha256:7ae36aab93ef9b6aaff02b37f8bb23df84bb2d7a3f6054ec8fc466072a648ce2
```

在 job `env` 中同时把 matrix target 显式映射为 `RUST_TARGET`；fetch 必须发生在 `pnpm test:rust` 之前，修复当前四平台缺 sidecar 的已知 CI 失败。
...
Expected: 本机无 Docker 时普通命令清楚报告 1 个环境跳过；设置 `LUMAROUTE_REQUIRE_CONTAINER=1` 时无 Docker 必须非零退出；`git diff --check` 无输出。

- [ ] **Step 5: Commit 强制 Jellyfin 契约**

```bash
git add tests/integration/support/jellyfin-container.ts \
  tests/integration/jellyfin-contract.test.ts \
  .github/workflows/quality.yml
git commit -m "test: require the live Jellyfin alpha contract"
```
...
**可独立验收：** Windows x64 生成 MSI/NSIS EXE，macOS Intel 与 Apple Silicon 各生成 DMG，Linux x64 生成 AppImage/deb；每个安装包有可验证 SHA-256 sibling，所有 runner 都先执行真实 mpv qualification 和自动启动冒烟，产物包含明确 Internal Alpha 限制标记。
...
**可独立验收：** 当前提交的 quality matrix 全绿，Linux live Jellyfin 未跳过；package matrix 四个 job 全绿并上传约定产物、checksum 和 marker；三个非 Apple Silicon mpv 目标有原生 runner 的真实 qualification 日志。
...
Expected: `pnpm check`、非容器集成场景和 Playwright 全部 PASS；若当前机器无 Docker，输出必须明确指出 live Jellyfin 的环境限制，且不能把本地 `pnpm test:integration` 记为完整 Jellyfin 通过；diff check 无输出。
...
Expected: 最新 run 对应 `git rev-parse HEAD`；Windows、macOS Intel、macOS Apple Silicon、Ubuntu 四个 job 全绿；Ubuntu 日志中 live Jellyfin 测试实际执行且无 skip；四个平台都在 Rust 测试前成功 fetch 对应 sidecar。
...
Expected: 四个 matrix job 全绿：

```text
x86_64-pc-windows-msvc: .msi + -setup.exe + each .sha256 + marker
x86_64-apple-darwin: .dmg + .sha256 + marker
...
Expected: `PASS verified 6 package checksum(s)`（Windows 2、macOS 2、Linux 2）。Universal DMG 不属于本阶段硬门，不应替代两个架构各自的 DMG。
...
Expected: `PASS mpv.lock.json manifest`；四个 target 都有可追溯原生 qualification run。
...
- [x] Temporary Jellyfin authenticates, browses, searches, plans playback, and reports progress.
  - Evidence: `tests/integration/jellyfin-contract.test.ts` required Linux container job; use the same successful quality run URL.
  - Limitations: controlled fixture and ephemeral credentials only.
...
- [x] Windows x64, macOS Intel/Apple Silicon, and Linux x64 artifacts have verified SHA-256 siblings.
...
## Task 6: 完成 macOS 与 Windows 的 Emby/Jellyfin 实机矩阵

**可独立验收：** macOS 与 Windows x64 分别用 Emby、Jellyfin 完成四个独立组合；每个组合验证安装、首次启动或安全凭证读取、浏览/详情、直放或不转码直接串流、暂停/恢复/跳转/停止、五类进度回写、退出后的 mpv/IPC 清理和卸载。私人地址、用户名、Token 不进入记录。

**Files:**
- Modify: `docs/release/v0.1-acceptance.md`

**Interfaces:**
- Consumes: Task 5 package run 的 exact artifact、SHA-256、Internal Alpha marker；测试人员本地安全环境中的 Emby/Jellyfin。
- Produces: 四条实机组合证据，每条都有日期、完整 OS 版本、安装包文件名或 artifact ID、结果和非敏感限制。
...
- [ ] macOS × Jellyfin
  - Evidence: not yet executed
  - Required: same closed loop as macOS × Emby
  - Limitations: execution environment not yet supplied
- [ ] Windows x64 × Emby
  - Evidence: not yet executed
  - Required: same closed loop as macOS × Emby
...
Expected: 四个组合都完成；进度通常与本地位置相差不超过 10 秒。硬件解码不可用时允许软件解码；无法播放时记录 OS、mpv build、媒体 codec 和脱敏错误类别，不记录 URL、用户名或 Token。
...
Expected: zero findings；diff check 无输出；四个组合都有实际证据或明确不通过原因。
...
**可独立验收：** 当前 HEAD 的本地质量门、强制容器 CI、package CI、artifact checksum、macOS/Windows 四个实机组合、真实 mpv 和泄漏扫描均有可追溯证据；只有全部通过时文档才写 `Internal Alpha: PASS`。
...
Expected: 所有命令 PASS；`git diff --check` 无输出；`git status --short` 只显示本任务对 acceptance 文档的预期修改。无 Docker 主机的本地 skip 不能替代 Task 5 Linux required-container CI。
...
3. Jellyfin container: Linux required-container test executed with zero skips.
4. Packages: six required installers and six valid checksum siblings present.
5. Real systems: macOS/Windows × Emby/Jellyfin four combinations passed.
6. Real mpv: H.264/H.265/AV1 + JSON IPC evidence for all four package targets, or a documented hardware limitation that does not invalidate software decode.
7. Credential leakage: zero findings in current quality run and acceptance document scan.
```

Expected: 七项全部有证据。任一缺失则最终结论必须是 `BLOCKED`，并保留未勾选项和明确原因。
...
**System evidence:** macOS/Windows × Emby/Jellyfin entries above
...
- [x] Spec §1：Tasks 1–7 分别覆盖 Emby/Jellyfin 闭环、真实 mpv、三平台产物、专用播放启动换线和唯一验收记录。
...
- [x] Spec §4.2：Task 2 固定 Jellyfin manifest digest，覆盖认证、浏览、搜索、播放计划和进度，Linux CI 设置强制运行时，skip 不算验收。
- [x] Spec §4.3：Task 3 对 H.264/H.265/AV1、启动/加载、暂停/恢复/跳转/停止、事件和敏感 header 泄漏建立 fail-closed 真实 mpv 证据。
- [x] Spec §5：Task 4/5 精确要求 Windows MSI+NSIS、两架构 macOS DMG、Linux AppImage+deb、每产物 SHA-256 和四条 Internal Alpha 警告。
- [x] Spec §6：Task 6 覆盖 macOS、Windows x64、Emby、Jellyfin 四组合及登录/凭证、浏览详情、直放/直接串流、控制、回写、清理、卸载；Linux 只保留 CI 自动证据。
...
- [x] Internal Alpha 七项最终条件在 Task 7 一一对应，签名、公证、公开许可证声明和普通用户分发明确延期且不阻塞内部 Alpha。
...
- [x] 任务顺序：错误契约与专用集成 → 强制 Jellyfin → 真实 mpv → package 门 → CI 证据 → 实机矩阵 → 最终判定；后续任务只消费前序产物。
