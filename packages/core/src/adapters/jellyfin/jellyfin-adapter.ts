...
import type {
  ...
  JellyfinItemsResultDto,
  JellyfinPlaybackInfoDto,
  JellyfinVirtualFolderDto,
  PublicSystemInfoDto,
} from './jellyfin-dto'
import {
  mapItem,
  mapJellyfinAuthSession,
  mapLibrary,
  selectPlaybackPlan,
  toProviderKind,
} from './jellyfin-mapper'

export class JellyfinAdapter implements AuthenticationAdapter, MediaServerAdapter {
  ...
  async authenticate(input: LoginInput): Promise<AuthSession> {
    ...
    return mapJellyfinAuthSession(info.data, auth.data)
  }
  ...
  async getLibraries(context: RequestContext): Promise<Library[]> {
    const response = await this.authorizedRequest<JellyfinVirtualFolderDto[]>(context, {
  ...
  async getItems(query: ItemQuery, context: RequestContext): Promise<Page<MediaItem>> {
    ...
    const response = await this.authorizedRequest<JellyfinItemsResultDto>(context, {
  ...
  async getPlaybackPlan(itemId: string, context: RequestContext): Promise<PlaybackPlan> {
    const response = await this.authorizedRequest<JellyfinPlaybackInfoDto>(context, {
